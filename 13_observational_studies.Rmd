...
Now let's examine a _natural_ experiment on school class size.  In Israel, it turns out, primary school class size is capped at 40 students.  This has an important consequence:  
...
Moreover, if we look at the `class` variable, it's also clear that green buildings are more likely to be fancy ["Class A" buildings](https://www.squarefoot.com/blog/class-a-buildings-class-c-buildings/), which are the most desirable properties in a given market:

```{r}
xtabs(~ class + green_rating, data=greenbuildings) %>%
  prop.table(margin=2)
```
...
```{r}
greenbuildings_matched = matchit(green_rating ~ age + class,
...
The average age is now nearly identical in the green and non-green groups.  Now let's look at class:  

```{r}
xtabs(~ class + green_rating, data=greenbuildings_matched) %>%
  prop.table(margin=2)
```
The green and non-green buildings have nearly identical proportions of class A, B, and C buildings.  

So once we've constructed the data set of matched pairs, the confounder variables (`age` and `class`) are much more closely balanced between the treatment and control groups.  Now we come to step 3: compare outcomes in the treated (green) and untreated (non-green) buildings, using only the matched data.  This is straightforward.  We'll do so using `t.test`, to calculate a large-sample confidence interval for the difference in average `rev_psf` between the two groups:  

```{r}
t.test(rev_psf ~ green_rating, data=greenbuildings_matched)
```

A comparison of revenue for this matched data set makes the premium for green buildings look a lot smaller: more like 75 cents, versus \$3.15 for the unmatched data.  Moreover, the confidence interval ranges from about -\$1.90 to \$0.40, indicating that for the matched data, we're not actually confident which group has a larger mean.  
...
## Study questions: observational studies  {-}  
