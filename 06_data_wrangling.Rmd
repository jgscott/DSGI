...
For our first example, we'll find the five coldest individual months in our data set on Rapid City, which spans the seventeen years from 1995 to 2011.

Anytime you're faced with a complex data-analysis task like this, I'd encourage you to remember our basic mantra of data science that we introduced all the way [back here](#sec_objects):

> Manage complexity by breaking complex tasks down into simple tasks, and then stitching the simple tasks together.  

For this analysis, our simple tasks look something like the following:  
...
Sometimes refining data is like [refining oil](https://www.eia.gov/energyexplained/oil-and-petroleum-products/refining-crude-oil-the-refining-process.php): when it comes to pipelines, the order in which you perform the steps really matters.  
...
Our goal for this data analysis is simple: make a line graph showing total toy imports over time across all categories, separately for each trading partner---but only for the top 5 partners by total dollar value of toys imported.

Let's break this down into simpler tasks.  Our first task is to actually find the top 5 partners by total dollar value.  This task itself can be broken down into simpler sub-tasks:  
...
Now we can look at the top 5 trading partners:

```{r}
head(country_totals, 5)
```

And we can even peel off the names of the countries using `select`:

```{r}
top_5_country_names = country_totals %>%
  head(5) %>%
  select(partner_name)
top_5_country_names
```

This list of names will be useful when we turn to our second main task: calculate toy imports over time across all categories, separately for each of these five trading partners.  Again, this task also breaks down into simpler subtasks:  

- filter the data set so that it includes only data points from the top 5 trading partners.  
- group the data points by trading partner and year  
- for each group, sum toy imports across all categories (i.e. lumping together everything from dolls to trains)  

We can accomplish this using `filter`, `group_by`, and `summarize`, storing the result in a summary table that we'll call `top5_byyear`:    

```{r, message=FALSE}
top5_byyear = toyimports %>%
  filter(partner_name %in% top_5_country_names) %>%
